...
public interface JpaUserFeatureRepository extends UserFeatureRepository, JpaRepository<User, Long> {
    ...
    @Query("SELECT new site.gnu_gongji.GnuGongji.dto.UserSubInfoDto(d.departmentId, d.departmentKo, d.departmentEng, us.startAt) FROM Department d " +
            "JOIN UserSub us ON us.departmentId = d.departmentId " +
            "JOIN us.user u " +
            "WHERE u.oauth2Id = :oauth2Id")
    List<UserSubInfoDto> findDepartmentsByUserOauth2Id(@Param("oauth2Id") String oauth2Id);
}
